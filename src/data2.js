...
export const filtradoC = (data, lenguaje, continente) => {
  return data.filter((country) => {
    for (const languageCode in country.languages) {
      ...
      if (
        country.languages[languageCode] === lenguaje &&
        country.continents.includes(continente)
      ) {
        return true;
      }
    }
...
export const filtradoE = (data, independiente) => {
  ...
  const filteredCountries = data.filter(
    (country) => country.independent === esIndependiente
  );
